...
// Regression: JS root-relative page path should be rewritten via runtime path helper
const testProcessJsFileRewritesRootPagePath = async () => {
  ...
  const processed = fs.readFileSync(jsPath, 'utf8');
  assert(processed.includes('window.__AWA_PATH__("projekt/projekthandboken")') || processed.includes("window.__AWA_PATH__('projekt/projekthandboken')"), 'JS page path should use runtime path helper');
